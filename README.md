...
| **Branches** | Local + remote, facet chips (All / Merged / Unmerged / Local-only / On remote), filter, current marker, unique commit counts, ahead/behind, tracking + merge badges, diff preview, branch commit list shortcut, multi-select bulk delete, remote-branch delete. |
...
| **Stash** | Save with message + include-untracked. Search by ref, message, branch, or age. Click any row to view changes (tree-style diff viewer). Apply / Pop / Drop. Multi-select bulk drop. |
